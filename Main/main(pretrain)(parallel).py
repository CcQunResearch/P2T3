...
from Main.utils import create_log_dict_pretrain_parallel, write_json, write_log
from Main.word2vec import Embedding, collect_sentences, train_word2vec
from Main.dataset import SequenceTreeDataset, IterableSequenceTreeDataset
...
def train(rank, ngpus_per_node, unlabeled_dataset_path, word2vec, bert_model, content_key, weight_path, log_path, args):
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:23451', world_size=ngpus_per_node, rank=rank)
    ...
    # data
    unlabeled_dataset = SequenceTreeDataset(unlabeled_dataset_path, d_model, embedding_method, word2vec,
                                            bert_model, content_key, use_chain_identifier, use_depth_embedding,
                                            use_type_embedding, max_sequence_length)
    train_sampler = DistributedSampler(unlabeled_dataset)
    collator = Collator(pooling, device)
    unsup_train_loader = DataLoader(unlabeled_dataset, pt_batch_size, collate_fn=collator, sampler=train_sampler)

    # unlabeled_dataset = IterableSequenceTreeDataset(unlabeled_dataset_path, d_model, embedding_method, word2vec,
    #                                         bert_model, content_key, use_chain_identifier, use_depth_embedding,
    #                                         use_type_embedding, max_sequence_length)
    # collator = Collator(pooling, device)
    # unsup_train_loader = DataLoader(unlabeled_dataset, pt_batch_size, shuffle=False, collate_fn=collator)
...
if __name__ == '__main__':
    ...
    log_dict = create_log_dict_pretrain_parallel(args)
    ...
    # dataset will be preprocessed and saved if not exist, preprocessed can't be done in the multi-process
    unlabeled_dataset = SequenceTreeDataset(unlabeled_dataset_path, d_model, embedding_method, word2vec,
                                            bert_model, content_key, use_chain_identifier, use_depth_embedding,
                                            use_type_embedding, max_sequence_length)

    world_size = ngpus_per_node
    mp.spawn(train, nprocs=world_size, args=(
        ngpus_per_node, unlabeled_dataset_path, word2vec, bert_model, content_key, weight_path, log_path, args))
